...
public class StreamerConfiguration {
    ...
    public static Optional<StreamerConfiguration> findStreamerConfiguration(MavenProject project, Log log) {
        if (!project.getResources().isEmpty()) {
            Optional<Path> maybeConfig = project.getResources()
                                                .stream()
            ...
            if (maybeConfig.isPresent()) {
                try {
                    return Optional.of(new StreamerConfiguration(mapper.readValue(readAllBytes(maybeConfig.get()), StreamerDefinition.class)));
                } catch (IOException e) {
